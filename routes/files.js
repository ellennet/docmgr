...
router.get('/:id', (req, res, next) => {
  ...
  if (config.oss) {
    co(function* () {
      username = yield token.ValidateToken(_token);
      fileUrl = oss.getFileUrl(id); //获取下载地址      
    }).then(() => {
    ...
    co(function* () {
      ...
      let jsonStr = JSON.stringify(doc.headers).replace('content-type', 'contentType');
      jsonStr = jsonStr.replace('content-disposition', 'contentDisposition');

      let obj = JSON.parse(jsonStr);

      let mime = obj.contentType;
      let cd = 'attachment;filename="' + urlencode(doc.ofilename) + '"';
          ...
          if (size === 0) {
            ...
            if (
              ...
              co(function* () {
                ...
                if (config.hasKb) {
                  //tika 抽取内容
                  //words = yield tikaCore.text(path);
                  //yield solrCore.addDoc(fileId,words);               
                }
